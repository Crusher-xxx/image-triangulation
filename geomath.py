...
class Camera:
    def __init__(self, geodetic, mtx, distortion_coeffs, resolution, cp1_geodetic, cp2_geodetic, cp1_pix, cp2_pix) -> None:
        self.mtx = mtx
        ...
        self.alpha = 0
        self.mtx_new, roi = cv.getOptimalNewCameraMatrix(self.mtx, self.distortion_coeffs, self.resolution, self.alpha, self.resolution)

        cp1_geodetic = np.array(cp1_geodetic)
        cp2_geodetic = np.array(cp2_geodetic)
        cp1_enu = np.array(pm.geodetic2enu(*cp1_geodetic, *self.geodetic))
        cp2_enu = np.array(pm.geodetic2enu(*cp2_geodetic, *self.geodetic))
        
        cp1_dirvec_cam = pixel2dirvec(cp1_pix, self.mtx_new)
        cp2_dirvec_cam = pixel2dirvec(cp2_pix, self.mtx_new)
    ...
    def pixel2dirvec_enu(self, pixel):
        dir_vec_cam = pixel2dirvec(pixel, self.mtx_new)
        dir_vec_enu = self.rot_mtx_cam2enu @ dir_vec_cam
        return dir_vec_enu
    
    def undistort(self, array: np.ndarray) -> np.ndarray:
        img_undistorted = cv.undistort(array, self.mtx, self.distortion_coeffs, None, self.mtx_new)
...
class Triangulator:
    def __init__(self, cam1: Camera, cam2: Camera) -> None:
        ...
        self.cam2_enu = pm.geodetic2enu(*self.cam2.geodetic, *self.cam1.geodetic)
    ...
    def triangulate(self, pix1, pix2):
        ...
        enu = np.mean([p1_enu, p2_enu], axis=0)
        geodetic = np.array(pm.enu2geodetic(*enu, *self.cam1.geodetic))
        return enu, geodetic
